-- Retry with explicit casts for calculate_title_from_rank
WITH totals AS (
...
SET 
  ...
  current_rank = r.user_rank::int,
  rank = public.calculate_title_from_rank((r.user_rank)::int, t.total_users),
  updated_at = now()
